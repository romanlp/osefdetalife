...
Single diner surface: a first-party public booking page at `/book/{slug}` (Angular SPA, mobile-first) plus a restaurant-facing management dashboard (Angular SPA, desktop-first). Dark mode supported from day one. `DESIGN.md` is the visual identity reference; this spine is the experience.

Diners reach the public booking page at `/book/{slug}` by opening a shared booking link (or scanning a QR code). The page renders with restaurant-specific colors. The dashboard is part of the same Angular app hosted on Firebase Hosting. Onboarding is required before dashboard access.
...
|---|---|---|
| Landing | Shared booking link (`/book/{slug}`) | Restaurant name + "Book a Table" CTA |
...
| Booking Link | Sidebar → Booking Link | Booking link, QR code, preview |
...
After completion: redirect to the Booking Link page showing the booking link and QR code. Onboarding skipped if restaurant profile already exists.
...
| Context | Do | Don't |
|---|---|---|
| Booking page landing | "Book a Table" | "Reserve your spot now!" |
...
|---|---|---|
| Booking page container | All booking steps | Full viewport. Restaurant colors applied via CSS custom properties. |
...
|---|---|---|
| Booking page loading | All steps | Spinner centered. No skeleton — steps are simple enough. |
| No availability | Time step | "No available times for this date." Suggest trying another date. |
| Invalid slug | Booking page | "Restaurant not found." No retry — slug is wrong. |
| Firebase error | Booking page | "Something went wrong. Please try again." Retry button. |
| Dashboard empty | Bookings list | "No bookings for this date." Neutral, not apologetic. |
| Onboarding complete | Booking Link page | Booking link with copy button and QR code. In-app preview of the booking page. |
...
- Focus management: booking page step transitions move focus to new step heading. Tab order follows reading order.
...
- Tap targets ≥ 44px (booking page) / 48px (dashboard).
...
### Flow 1 — Diner books a table (Alex, Tuesday evening, via shared booking link)

1. Alex opens the restaurant's shared booking link.
2. The public booking page loads at `/book/{slug}`: restaurant name, "Book a Table" button.
...
6. **Climax:** Booking Link page shows the booking link and QR code. Maria copies the link and shares it with diners. The public booking page is live at `/book/la-trattoria-london`.
